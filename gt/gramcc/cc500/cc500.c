...
char *my_realloc(char *old, int oldlen, int newlen){
   ...
   printf ("my_realloc: size=%d\n", newlen );
   //printf ("code_size=%d codepos=%d \n",code_size, codepos);	
...
void emit (int n, char *s){
  ...
  printf ("emit: code_size=%d codepos=%d \n",code_size, codepos);	
...
void be_start()
{
  ...
  sym_define_global( sym_declare_global("exiZ") );
  /* pop %ebx ; pop %ebx ; xor %eax,%eax ; inc %eax ; int $0x80 */
  emit(7, "\x5b\x5b\x31\xc0\x40\xcd\x80");

  sym_define_global(sym_declare_global("getchaZ"));
  ...
  sym_define_global(sym_declare_global("malloZ"));
  /* mov 4(%esp),%eax */
  ...
  sym_define_global(sym_declare_global("putchaZ"));
...
// #importante
// Tô achando que isso coloca o código no stdout.
// Se for, podemos trocar o putchar por fputc (stdout, ch) e 
// colocarmos os bytes dentro do stdout. depois salvar.
...
// #todo
// Ele pega coisas no stdin e coloca no stdout.
// Então temos que configurar o stdin. pegando um
// arquivo do mesmo jeito que o gramc faz.
// ele usa o comando > ccxxx para canalizar a stdout para
// um novo arquivo.
// então precisamos ao fim canalizarmos salvar stdout assim como fazemos
// no gramcode.

int cc500_main ( int argc, char *argv[]){
	
    printf ("cc500: Initializing ...\n");
    
    // #todo
    // abrir um arquivo.
	
	// ??
	// Não seo o que é isso.
	// Será a base do arquivo executável.
	// temos que mudar para outra lugar.
    ...
    //#bugbug
    //antes disso temos que configurar o stdin
    //para apontar para o arquivo em c que carregaremos com fopen. 
    //>>> #obs: já estamos fazendo isso em main()
